...
AREA_SUMMARY = {
    ...
    "impact": "외부TR(경쟁사 확산), TR(피인용 영향력), 기술 선도성을 본다.",
}
...
COMPONENT_INFO: Dict[str, Dict[str, Any]] = {
    ...
    "market.entry": {
        ...
        "how": "패밀리에 **해당 국가 출원이 존재하는지(유무)** 만 보고 가중치를 합산한다. "
               ...
               "'WIPS패밀리 개별국 문헌 수(출원기준)' 컬럼을 1순위 근거로 사용하며, "
               "**PCT/EPC 지정국은 포함하지 않는다**(지정국은 출원할 수 있는 나라 목록일 뿐 "
               "실제 출원이 아니므로, 포함하면 PCT 1건이 수십 개국 출원처럼 계산된다). "
               "각 국가가 어느 컬럼에서 왔는지는 결과 상세의 countrySources 에서 확인할 수 있다.",
    ...
    "impact.competitorCoverage": {
        "label": "경쟁사 커버리지",
        "how": "윈텔립스의 '외부TR'(외부 인용 지표) 값을 그대로 쓴다. 백분위가 아니라 "
               "**업로드 모집단에서 가장 높은 외부TR 대비 비율**로 채점하므로, 1등 문헌이 만점을 "
               "받고 나머지는 1등 대비 몇 %인지가 그대로 점수가 된다. 값이 비어 있으면 0점이다.",
        "formula": "(대상 건의 외부TR ÷ 모집단 외부TR 최대값) × 7",
        "fields": ["외부TR"],
    },
    "impact.citation": {
        "label": "연령보정 피인용 영향력",
        "how": "윈텔립스의 'TR' 값을 그대로 쓴다(연령보정은 TR 지표 자체에 반영된 것으로 본다). "
               "**업로드 모집단에서 가장 높은 TR 대비 비율**로 채점한다. 값이 비어 있으면 0점이다.",
        "formula": "(대상 건의 TR ÷ 모집단 TR 최대값) × 13",
        "fields": ["TR"],
    },
    "impact.leadership": {
        "label": "기술 선도성",
...
RULES = [
    ...
    {
        "title": "③ 모집단 최대값 대비 비율",
        "why": "외부TR·TR 은 윈텔립스가 이미 가공해 놓은 지표라, 순위(백분위)보다 "
               "'1등 대비 얼마나 되는가' 를 그대로 쓰는 편이 값의 크기 차이를 살린다.",
        "how": "업로드 모집단(채점 대상 문헌)에서 해당 지표의 최대값을 구하고, "
               "(대상 건의 값 ÷ 최대값) × 배점 으로 채점한다. 값이 비어 있으면 0점이다.",
    },
    {
        "title": "④ Gate 1 — 주제 적합도",
    ...
    {
        "title": "⑥ 출원인 표준화는 그룹별 승인이 필요하다",
        "why": "같은 회사가 여러 표기로 흩어져 있으면 출원인 기준 통계(시장 영향력, 자기인용 "
               "판정)가 어긋난다. 반대로 서로 다른 회사가 잘못 묶이면 점수가 부풀려진다.",
        "how": "업로드 직후 표기를 자동으로 묶어 후보를 보여 준다. 표준명은 **한글 표기를 먼저** "
               "추천하고 주식회사·Inc.·Co.,Ltd. 같은 법인격 표기를 떼어 핵심 명칭만 남긴다. "
               "표기가 2개 이상 묶인 그룹은 [승인] 을 눌러야 병합되며, 승인하지 않은 그룹은 "
               "원본 표기 그대로 분석된다. [변경] 으로 표준명을 직접 고칠 수 있다.",
        "note": "공동출원 행의 '대표명화' 값은 출원인 수와 개수가 맞을 때만 각 출원인에게 붙인다. "
                "행 단위 값을 모든 출원인에게 붙이면 공동출원 파트너가 대표 출원인과 한 그룹으로 "
                "병합된다. 그래도 잘못 묶인 표기는 × 버튼으로 분리할 수 있다.",
    },
    {
...
STEPS = [
    ...
    ("2. 출원인 표준화", "같은 회사의 여러 표기(삼성전자(주) / 삼성전자 주식회사 / SAMSUNG "
                        "ELECTRONICS CO., LTD.)를 하나로 묶고, 한글 핵심 명칭(삼성전자)을 "
                        "표준명으로 추천한다. 그룹별 [승인] 을 누른 것만 병합되며, "
                        "승인하지 않은 그룹은 원본 표기를 그대로 쓴다."),
