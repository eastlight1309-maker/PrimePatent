...
패키지 31개 모듈이 파일 안에 압축 포함되어 있어 별도 배치가 필요 없습니다.
...
2. **출원인 표준화** — 같은 회사의 여러 표기(삼성전자(주) / 삼성전자 주식회사 /
   ...
   표준명은 **한글 표기를 먼저** 추천하고, 주식회사·Inc.·Co.,Ltd. 같은 법인격 표기를 떼어
   **핵심 명칭만**(삼성전자) 남깁니다. 단독출원·공동출원 모두 각 출원인을 따로 셉니다.
   표기가 2개 이상 묶인 그룹은 그룹별 **[승인]** 을 눌러야 병합되고, **[변경]** 으로 표준명을
   직접 고칠 수 있습니다. 승인하지 않은 그룹은 원본 표기를 그대로 사용합니다.

   > 공동출원 행의 '출원인 대표명화' 값은 출원인 수와 개수가 맞을 때만 각 출원인에게
   > 붙입니다. 행 단위 값을 모든 출원인에게 붙이면 공동출원 파트너가 대표 출원인과
   > 한 그룹으로 병합됩니다(무관한 회사가 묶이는 원인).
...
| 영향력·경쟁성 | 25 | 경쟁사 커버리지 7(외부TR) · 연령보정 피인용 13(TR) · 기술 선도성 5 |
...
- **모집단 최대값 대비 비율**: 외부TR·TR 은 백분위가 아니라 `(값 ÷ 모집단 최대값) × 배점` 으로 채점
