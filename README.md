...
```
총점 81 = 권리 중요도 23 + 기술 중요도 26 + 시장 중요도 20 + 영향력·경쟁성 12
        = 정량점수 52 + LLM 분석점수 29
```
...
| 영역 | 배점 | 세부지표(배점) |
|---|---|---|
| 권리 중요도 | 23 | 생존성 8 · 청구범위 강도 8(정량4+LLM4) · 잔존기간 4 · 권리유지·방어 3 |
| 기술 중요도 | 26 | Topic 적합도 8 · 핵심 기술기여도 8 · 문제·효과 6 · 범용성 4(LLM3+CPC1) |
| 시장 중요도 | 20 | 주요시장 진입도 8 · 출원인 영향력 5 · 상업화·거래 4 · 경쟁사 커버리지 3 |
| 영향력·경쟁성 | 12 | 연령보정 피인용 8 · 기술 원천성 4 |
...
```bash
python -m unittest discover -s tests -p "test_*.py" -v   # 단위 + 통합 + DSS 기동 + 회귀 (100건)
